...
interface Props {
  searchParams: { sort?: string; genre?: string };
}

export default async function BrowsePage({ searchParams }: Props) {
  const { sort = 'terbaru' } = searchParams;
  let items: Donghua[] = [];

  try {
    const data = await getHome(2);
    if (sort === 'populer') {
      items = (data.populer || data.slider || data.popular || []) as Donghua[];
    } else if (sort === 'tamat') {
      items = (data.tamat || data.completed || []) as Donghua[];
    } else {
      items = (data.terbaru || data.ongoing || data.latest || []) as Donghua[];
    }
    if (!items.length && data.data) items = data.data as Donghua[];
  } catch (err) {
