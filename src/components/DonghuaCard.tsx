...
import { getCoverImg, getTitle, getItemSlug, getStatus, getEpDisplay } from '@/lib/utils';
    ...
    return (
      <Link href={href}
        ...
        <div className="flex-1 min-w-0">
          ...
          {ep && <p className="text-xs mt-1 font-medium" style={{ color:'var(--cyan)' }}>{ep}</p>}
          <p className="text-xs mt-0.5 font-medium"
            style={{ color: isOngoing ? 'var(--rose)' : 'var(--muted)' }}>{status}</p>
        </div>
        <Play size={14} style={{ color:'var(--faint)' }} className="flex-shrink-0" />
      </Link>
    );
  }
