...
import { getTitle, getCoverImg, getSynopsis, getGenresArr, getTotalEp, getStatus, getStatusColor, buildEpSlug, getEpDisplay } from '@/lib/utils';
...
export default async function DetailPage({ params }: Props) {
  ...
  catch (err) { console.error('[DETAIL]', err); notFound(); }
      ...
      <div className="px-4 -mt-16 relative z-10">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-2">
              ...
              {donghua.score && (
                <span className="flex items-center gap-1 text-xs font-semibold" style={{ color:'var(--amber)' }}>
                  <Star size={10} fill="var(--amber)" />{donghua.score}
                </span>
              )}
              <span className="flex items-center gap-1 text-xs" style={{ color:'var(--muted)' }}>
                <Tv size={10} />{getTotalEp(donghua)} Ep
              </span>
              {(donghua.year ?? donghua.tahun) && (
                <span className="flex items-center gap-1 text-xs" style={{ color:'var(--muted)' }}>
                  <Calendar size={10} />{donghua.year ?? donghua.tahun}
                </span>
              )}
            </div>
          </div>
        ...
        {genres.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            {genres.map(g => (
              <span key={g} className="chip" style={{ fontSize:11, padding:'4px 12px' }}>{g}</span>
            ))}
          </div>
        )}
        ...
        <div className="flex gap-3 mb-8">
          {episodes.length > 0 && (
            <Link href={"/watch/" + firstSlug}
              ...
              <Play size={16} fill="#000" />Tonton Ep 1
            </Link>
          )}
          {episodes.length > 1 && (
            <Link href={"/watch/" + lastSlug}
              className="flex items-center justify-center gap-1.5 px-4 py-3.5 rounded-2xl font-medium text-sm"
